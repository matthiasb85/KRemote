...
typedef struct
{
  ...
  uint32_t pwm_out_min[RC_OUTPUT_AN_MAX];
  uint32_t pwm_out_max[RC_OUTPUT_AN_MAX];
  uint32_t loop_cmd_period_ms;
}rc_output_config_t;

#define RC_OUTPUT_PWM_MS_TO_TICKS(x)  ((RC_OUTPUT_PWM_TIMER_FREQ/1000)*(x))
#define RC_OUTPUT_PWM_US_TO_TICKS(x)  (uint32_t)((((float)RC_OUTPUT_PWM_TIMER_FREQ)/1000.0)*(((float)x)/1000.0))
